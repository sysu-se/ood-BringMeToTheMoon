---
name: sudoku-hw2-hint-explore-final
overview: 为数独游戏实现提示(Hint)和探索(Explore)功能。提示分为规则法候选数和求解器答案；探索模式使用独立子会话和分支历史
todos:
...
在 Homework 1 已有的 Sudoku/Game 对象模型基础上，增加提示（Hint）和探索（Explore）两个功能，保持对象模型整体一致，通过 EVOLUTION.md 文档说明新增功能如何影响设计。
...
- 直接得到指定格子的正确答案
- 通过调用已有的 `solveSudoku()` 函数（来自 `src/node_modules/@sudoku/sudoku.js`，内部使用 `@mattflow/sudoku-solver`）
- 输入：行列坐标 (row, col)，范围 [0-8]
...
- 继续使用现有项目的技术栈，保持一致性
- 候选提示用规则法（在 Sudoku.js 中实现，不依赖外部库）
...
3. 收集同一行已有的数字（排除 0）
4. 收集同一列已有的数字（排除 0）
5. 收集同一宫（3x3 盒）已有的数字（排除 0）
6. 从 1-9 中排除以上所有数字，得到候选数集合
7. 返回 Set<number>
...
- 实现 `getCandidates(row, col)` 方法，使用规则法计算候选数集合
